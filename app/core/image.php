...
class image
{
    public function moveImageFromTempToPublic($input, $imageName, $suffix, $paths)
    {
...
//            $paths['system_public_temp_path']
            ...
            $this->makeDirectory($paths['system_public_path']);
...
//            Log::debug('$moveTo exist: ' . File::exists($moveTo));
     ...
     * @return mixed
     */
    public function saveImages($input, $persistent = false){

        Log::debug('saveImages');

//        dd($input);
        //Process images in input
        $input = $this->saveImage($input, 'image1', $persistent);
        $input = $this->saveImage($input, 'image2', $persistent);
        $input = $this->saveImage($input, 'image3', $persistent);
        $input = $this->saveImage($input, 'image4', $persistent);
        $input = $this->saveImage($input, 'image5', $persistent);
        $input = $this->saveImage($input, 'image6', $persistent);
        //dd($input);
     ...
     * @return mixed
     */
    private function saveImage($input, $image_name, $persistent)
    {
...
//        dd($input, $image_name, $persistent, $paths = $this->getPaths(), env('PUBLIC_PATH'), $system = env('SYSTEM_PATH'));
...
//            $month = date('m');
            ...
            $input = $this->moveImageFromTempToPublic($input, $image_name, '_thumb',$paths);
            $input = $this->moveImageFromTempToPublic($input, $image_name, '_std', $paths);
            $input = $this->moveImageFromTempToPublic($input, $image_name, '_full', $paths);
        }
        else if (isset($input[$image_name]) && $input[$image_name] != "")
        {
            ...
            $input = $this->saveStd($input, $rdm, $image_name, $paths, $persistent);
            $input = $this->saveThumb($input, $rdm, $image_name, $paths, $persistent);
            $input = $this->saveFull($input, $rdm, $image_name, $paths, $persistent);
        }
    ...
    private function saveStd($input, $rdm, $image_name, $paths, $persistent)
    {
        $file_name = $rdm . '_std_' .  $input[$image_name]->getClientOriginalName();

        //Resize image to max 700 width keeping aspect ratio
        $image = interventionImage::make($input[$image_name]->getRealPath())->resize(700, null, function ($constraint) {
            $constraint->aspectRatio();
            $constraint->upsize();
        });
    ...
    private function saveFull($input, $rdm, $image_name, $paths, $persistent)
    {
        $file_name = $rdm . '_full_' .  $input[$image_name]->getClientOriginalName();

        $image = interventionImage::make($input[$image_name]);
