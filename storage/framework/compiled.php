...
class Kernel implements KernelContract
{
    ...
    protected function sendRequestThroughRouter($request)
    {
        ...
        $this->bootstrap();
        return (new Stack($this->app))->send($request)->through($this->middleware)->then($this->dispatchToRouter());
    }
        ...
        $this->app['events']->listen('router.matched', function () {
            $this->app->resolvingAny(function ($resolved, $app) {
                if ($resolved instanceof FormRequest) {
                    ...
                    $resolved->setContainer($app)->setRedirector($app['Illuminate\\Routing\\Redirector']);
                }
            });
        });
    }
...
class Router implements RegistrarContract
{
    ...
    protected function runRouteWithinStack(Route $route, Request $request)
    {
        ...
        return (new Stack($this->container))->send($request)->through($middleware)->then(function ($request) use($route) {
...
class ControllerDispatcher
{
    ...
    protected function callWithinStack($instance, $route, $request, $method)
    {
        ...
        return (new Stack($this->container))->send($request)->through($middleware)->then(function ($request) use($instance, $route, $method) {
...
class Stack
{
    ...
    public function then(Closure $app)
    {
        ...
        return call_user_func(array_reduce($middleware, $this->getSlice(), $firstSlice), $this->request);
    }
...
class Dispatcher implements DispatcherContract
{
    ...
    {
        try {
            return (new ReflectionClass($class))->implementsInterface('Illuminate\\Contracts\\Events\\QueuedHandler');
        } catch (\Exception $e) {
            return false;
        }
    }
    protected function createQueuedHandlerCallable($class, $method)
    {
        return function () use($class, $method) {
            $this->resolveQueue()->push('Illuminate\\Events\\FireQueuedHandler', array('class' => $class, 'method' => $method, 'data' => func_get_args()));
        };
    }
    public function forget($event)
    {
